# Predictive Maintenance and Condition Monitoring at Grunnåi Power Plant

Exploratory data analysis and modeling workflow for Grunnåi hydropower signal data.

This repository contains helper functions and example notebooks for loading time-series signal exports, cleaning invalid measurements, identifying operating periods, detecting steady-state windows, producing plots, and building a modeling dataset for Drive-End (DE) vibration analysis.

## Repository Contents

- `environment.yml` - Conda environment definition for the project.
- `helper_functions.py` - Shared functions for loading data, cleaning signals, detecting operating periods, extracting steady-state windows, and building modeling datasets.
- `Examples/` - Example notebooks showing the analysis workflow:
  - `plot_raw.ipynb` - Plotting notebook for raw signal data.
  ...
  - `FeatureExtraction.ipynb` - Window feature extraction, steady-state detection, and modeling dataset creation.
...
1. `Examples/plot_raw.ipynb`
2. `Examples/EDA.ipynb`
3. `Examples/PELT.ipynb`
4. `Examples/FeatureExtraction.ipynb`
5. `Examples/modeling.ipynb`

The exact order may depend on which intermediate files have already been generated locally.

## What This Repository Does

This project analyzes operational signal data from the Grunnåi power plant. It focuses on the relationship between generator/turbine operating conditions and DE bearing vibration.
...
Some notebooks expect local data folders containing `.csv` files exported from an external monitoring or control system. These raw data files are not included in the public repository.

Local file paths may need to be adjusted before running the notebooks.
...
- ruptures: Truong, C., Oudre, L., and Vayatis, N. (2020). Selective review of offline change point detection methods. *Signal Processing*, 167, 107299. <https://doi.org/10.1016/j.sigpro.2019.107299>
- dcor: Ramos-Carreño, C., and Torrecilla, J. L. (2023). dcor: Distance correlation and energy statistics in Python. *SoftwareX*, 22, 101326. <https://doi.org/10.1016/j.softx.2023.101326>
